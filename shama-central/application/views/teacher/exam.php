		    	...
		    	<?php $attributes = array('role'=>'form','name' => 'quizform', 'id' => 'quizform','class'=>'form-container-input');
	                ...
	                <div class="row">
	                	...
	                	<div class="col-lg-6">
                			...
                			<input type="text" id="inputquizdate" name="inputquizdate" placeholder="Quiz Date"  tabindex="1" value="" required="required">
	                	</div>
	                </div>
	                <div class="row">
	                	<div class="col-lg-6">
	                		<div class="upper-row">
	                			<label><span class="icon-address"></span> Class <span class="required">*</span></label>
                				 ...
                				 <select name="inputclass" id="inputclass" >
	                					<?php 
	                						...
	                						if(count($classlist))
	                						{
	                							...
	                							foreach ($classlist as $key => $value) {
	                								?>
	                									 <option  value="<?php echo $value->class_id; ?>" <?php if(isset($schedule_single) && ($schedule_single[0]->class_id == $value->class_id)){echo 'selected';}?> required="required"><?php echo $value->grade; ?></option>
	                								<?php
	                							}
	                						}
	                					?>
                  	...
                  	<div class="row">
                		...
                		<div class="upper-row">
							...
							<input type="radio" name="input_term_type"  value="bt" <?php if(isset($schedule_single) && $schedule_single[0]->quiz_term == 'bt'){echo "checked='checked'";}?> <?php if( ! isset($schedule_single) ){echo "checked='checked'";}?> >Before Midterm <br>
                            <input type="radio" name="input_term_type"  value="at" <?php if(isset($schedule_single) && $schedule_single[0]->quiz_term == 'at'){echo "checked='checked'";}?>> After Midterm
            			</div>
                	</div>				                
		             <div class="row">
                		<div class="col-lg-3">
                			<br>
                			<input type="submit" value="Save" class="btn btn-default sm" id="savequiz" name="inputQuizBtn">
				...
				httprequest('<?php echo $path_url; ?>getquestionlist',({id:$scope.lastid})).then(function(response){
					if(response.length > 0 && response != null)
					{
						...
						for (var i = 0; i < response.length; i++) {
                            ...
                            {
                                cont_str += '<img onclick="angular.element(this).scope().showbigger(\''+response[i].image_src+'\');" src="'+response[i].thumbnail_src+'" class="quiz-image" width="50"/>';
                            }
                            cont_str += '</td>'
                            cont_str += '<td>'
                            ...
                            if(response[i].quiz_type == 't'){
                                ...
                                for (var k = 0; k < response[i].options.length; k++) {
                                    cont_str += '<td><img onclick="angular.element(this).scope().showbigger(\''+response[i].options[k].image_src+'\');" src="'+response[i].options[k].option+'" width="50"/></td>'
                                }
                            }
							...
							cont_str += '<span aria-hidden="true" class="glyphicon glyphicon-remove"></span>'
							cont_str += '</a>'
...
setTimerForWidget('section',1)
	...
	function setTimerForWidget(crname,ctime)
    {
      	...
      	try{
			...
			httprequest('<?php echo $path_url; ?>getsubjectlistbyclass',data).then(function(response){
				if(response.length > 0 && response != null)
				{
					...
					var selected_subject = '<?php if($this->uri->segment(2)){ echo $this->uri->segment(2) ; } ?>'
					if(selected_subject != '' )
					{
						getSelectedSubject(parseInt(selected_subject))
					}
				}
				else{
					$scope.subjectlist = [];
  	...
  	function getSelectedSubject(rowid)
  	{
  		try{
