...
(function ($)
{
    $.fn.sortableLists = function (options)
    {
        ...
        function showOnTopPlus(e, oEl, outside)
        {
            ...
            console.log(e.pageX,oEl.offset().left)
            // Hint inside the oEl
...
function MenuEditor(idSelector, settings) {
    ...
    function addItem() {
       ...
       // var btnEdit = TButton({classCss: 'btn btn-default btn-xs btnEdit', text: labelEdit});
        ...
        //var grpBtns = $("<div>").addClass('btn-group pull-right').append(btnRemv);
    ...
    function createMenu (arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var $li = $('<li>');
            $li.attr('id', 'less_'+v.lessonid);
            $li.attr('data-subject-id', v.subjectid);
            $li.attr('data-lesson-id', v.lessonid);
            $li.attr('data-lesson-posistion', v.lessonposition);
